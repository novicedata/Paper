...
- 기존 CAT은 IRT 기반 접근이 주류이나, 강한 가정(단일 차원성·지역 독립성)에 의존하여 실제 환경 적용에 한계가 존재함.
- 트리 기반 CAT이 대안으로 제시되었으나, 과적합과 항목 노출 문제 등 구조적 제약이 있음.
- 부스팅 모델은 높은 예측 정확도를 제공하지만, 다수의 얕은 트리 구조로 인해 CAT에 직접 적용하기 어려움.
- 본 연구는 부스팅 모델을 개별 트리 단위로 분해하고 부분집합으로 재구성한 Grove 알고리즘을 제안함.



* **주요 활동**
  * 부스팅 기반 CAT 구조 설계
    * CatBoost 모델 학습 후 개별 트리 분해 및 서브그룹 조합을 통해 Grove 알고리즘 구조 설계 및 구현
  * 실제 데이터 기반 검증 실험
    * VIQ, TMAS, KMLE 데이터에 Grove를 적용하여 기존 DT 기반 CAT 및 CatBoost 모델과 성능 비교
  * 정량 평가 및 구조 분석
    * ROC-AUC, 정확도, 과적합 비교, 항목 노출 분산 효과 분석 및 CAT 경로 시각화 수행
